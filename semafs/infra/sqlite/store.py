...
class SQLiteStore(NodeStore):
        ...
        if cursor.fetchone() is None:
            ...
            cursor.execute(
                ...
                INSERT INTO nodes(
                    ...
                    content, summary, payload, tags, is_archived, created_at, updated_at
    ...
    def _save_sync(self, node: Node) -> None:
        ...
        if exists:
            ...
            cursor.execute(
                ...
                INSERT INTO nodes (
                    id, parent_id, name, canonical_path, node_type, stage,
                    content, summary, payload, tags, is_archived,
    ...
    def _all_paths_sync(self) -> frozenset[str]:
        ...
        if rows:
            return frozenset(row["canonical_path"] for row in rows)
        cursor.execute("SELECT canonical_path FROM nodes WHERE is_archived = 0")
