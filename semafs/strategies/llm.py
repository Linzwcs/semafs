...
from ..core.ops import (AnyOp, MergeOp, MoveOp, PersistOp, RebalancePlan,
                        SplitOp, UpdateContext)
    ...
    for item in raw_ops:
        try:
            ...
            match op_type:
                ...
                case OpType.SPLIT:
                    if len(ids) < 2:
                        logger.warning("SPLIT ids 不足 2 个，跳过")
                        continue
                    ops.append(
                        SplitOp(
                            ids=ids,
...
class HybridLLMStrategy(LLMStrategy):
    ...
    2. 有 inbox 但总数少（< threshold）→ 规则策略（不调 LLM）
    ...
    async def create_plan(self, context: UpdateContext,
        ...
        if not force_llm:
            if not context.pending_nodes and total_nodes <= self._max_nodes:
                return None  # 无新碎片且未满，直接跳过

            if total_nodes <= self._max_nodes:
                logger.info("目录未达上限 (%d/%d)，采用规则降级", total_nodes,
                            self._max_nodes)
                return self.create_fallback_plan(context)
    ...
    def create_fallback_plan(self, context: UpdateContext) -> RebalancePlan:
        """
